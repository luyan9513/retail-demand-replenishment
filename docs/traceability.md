...
| DuckDB、dbt-duckdb、Python、SQL、Pandas、sklearn、Plotly、Streamlit | `README.md`、`docs/02_solution_architecture.md` | `src/`、`sql/`、`dbt/`、`app/app.py` | uv Python 3.12；`dbt build` 24/24；Streamlit 本地健康检查 `ok` | 已验证；依赖不在本次发布中新增 |
| 取消单、数量/价格异常、重复、稀疏审计 | `docs/03_data_design.md`、`docs/data_card.md` | `sql/01_quality_checks.sql`、`sql/03_duplicate_sensitivity.sql`、dbt intermediate、`src/run_pipeline.py` | 11 项质量审计 + 9 项重复敏感性；默认有效 1,041,670、严格 1,007,913；取消 19,494、候选重复 34,335、稀疏 SKU 3,135 | 已验证；候选重复不等于确认的系统重复，默认保留并提供严格对照 |
...
| 基线与机器学习候选 | `docs/04_model_and_experiment_design.md`、`docs/model_card.md`、`docs/07_code_and_data_lineage_guide.md` | `src/backtest.py`、`src/train.py` | `backtest_predictions.csv`、`model_metrics.csv` | 已验证；seasonal/移动平均/Croston-SBA/共享 HGB 均被回测，未来 HGB 已复用共享路径 |
...
| WAPE、MAE、sMAPE、偏差及分层误差 | `docs/01_requirements_and_metrics.md`、`docs/metrics_dictionary.md` | `src/metrics.py`、`tests/test_metrics.py` | 总体/窗口/分层/SKU 四类 metrics CSV；HGB WAPE 0.7534、MAE 71.7433、偏差 0.0239 | 已验证；HGB sMAPE 不是最佳，故不宣称全面胜出 |
| 安全库存、ROP、建议补货量与敏感性 | `docs/assumptions.md`、`docs/01_requirements_and_metrics.md` | `src/replenishment.py`、`src/scenarios.py`、`tests/test_replenishment.py` | `replenishment_scenarios.csv`；默认合计 SS 12,199.43、ROP 26,867.41、建议量 26,879；库存位置/MOQ/箱规均可配置 | 已验证；均为用户参数驱动的模拟 |
...
| 2026-08-02 | 可投递性改进发布 | 消除候选重复默认删除、HGB 前后不一致、间歇基线缺失和补货输入过简化等问题 | 默认保留候选重复并导出敏感性；新增 Croston-SBA；统一 HGB；加入库存位置/MOQ/箱规/提前期波动；补端到端测试和看板下载 | `docs/logs/2026-08-02_improvement_release.md`、`docs/09_improvement_release_notes.md` | 否；17 pytest、24 dbt 检查和真实管道均通过，但公开数据边界仍存在 |
| 2026-08-02 | 提交与远端交付 | 需要把可验证改进交给可访问仓库，而非只留在本机 | 提交 `8f07400` 并推送 `origin/main`；未加入原始数据、DuckDB、处理产物或本机 dbt 配置 | Git 推送返回 `6fc06dc..8f07400 main -> main` | 否；远端 CI 的实际运行状态仍需在 GitHub 页面观察 |
| 2026-08-04 | 工作区缓存与服务清理 | 避免可再生产物和已停止的本地服务残留 | 删除项目级 Python/pytest/dbt 缓存与 `.DS_Store`，保留 `.venv`、原始数据、DuckDB 和处理结果；检查 8502 端口 | `docs/logs/2026-08-04_workspace_cleanup.md`；缓存目录不存在，8502 无监听，Git 工作区干净 | 否；下次运行工具会按预期重新生成缓存 |
